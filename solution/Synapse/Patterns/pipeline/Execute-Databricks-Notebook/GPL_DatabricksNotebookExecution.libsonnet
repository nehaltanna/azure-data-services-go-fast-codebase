function(GenerateArm="false",GFPIR="{IRA}",SourceType="",SourceFormat="", TargetType="", TargetFormat="", SparkPoolName = "")
...
local name = "GPL_" + "DatabricksNotebookExecution_" + "Primary_" + GFPIR ; 
        ...
        "activities": [
            ...
            },
            {
                ...
                "typeProperties": {
                    ...
                    "parameters": {
                        "Body": {
                            "value": "@json(concat('{\"TaskInstanceId\":\"', string(json(string(pipeline().parameters.TaskObject)).TaskInstanceId), '\",\"ExecutionUid\":\"', string(json(string(pipeline().parameters.TaskObject)).ExecutionUid), '\",\"RunId\":\"', string(pipeline().RunId), '\",\"LogTypeId\":1,\"LogSource\":\"ADF\",\"ActivityType\":\"Execute Databricks Notebook\",\"StartDateTimeOffSet\":\"', string(pipeline().TriggerTime), '\",\"EndDateTimeOffSet\":\"', string(utcnow()), '\",\"Comment\":\"', encodeUriComponent(string(activity('Execute Databricks Notebook').error.message)), '\",\"Status\":\"Failed\"}'))",
                            "type": "Expression"
                        },
                        "FunctionName": "Log",
                        "Method": "Post"
            ...
            },
            {
                ...
                "typeProperties": {
                    ...
                    "parameters": {
                        "Body": {
                            "value": "@json(concat('{\"TaskInstanceId\":\"', string(json(string(pipeline().parameters.TaskObject)).TaskInstanceId), '\",\"ExecutionUid\":\"', string(json(string(pipeline().parameters.TaskObject)).ExecutionUid), '\",\"RunId\":\"', string(pipeline().RunId), '\",\"LogTypeId\":1,\"LogSource\":\"ADF\",\"ActivityType\":\"Copy Blob to Blob\",\"StartDateTimeOffSet\":\"', string(pipeline().TriggerTime), '\",\"EndDateTimeOffSet\":\"', string(utcnow()), '\",\"RowsInserted\":\"', string(activity('Execute Databricks Notebook').output.runPageUrl), '\",\"Comment\":\"\",\"Status\":\"Complete\"}'))",
                            "type": "Expression"
                        },
                        "FunctionName": "Log",
                        "Method": "Post"
